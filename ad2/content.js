...
function checkAd() {
  const adContainer = document.querySelector('.ad-container, .ad-interrupting'); // Adjust these selectors as needed
  const videoPlayer = document.querySelector('video');

  if (videoPlayer) {
    let isAdPlaying = adContainer && (adContainer.style.display !== 'none' || adContainer.classList.contains('ad-interrupting'));
